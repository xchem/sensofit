...
that fails any current QC rule is rejected; no linear drift correction or QC
rescue is applied. The current rules are: baseline standard deviation ≤2.5 RU;
...
The final fitting path therefore uses the measured raw blank in:

$$
\text{signal\_dr} = (\text{signal\_sample} - \text{baseline\_sample}) - (\text{signal\_blank} - \text{baseline\_blank})
$$

Earlier guarded linear-drift experiments were removed from the final method:
blank-only detrending could add an artificial slope to the doubly referenced
sample trace. Plotting remains backward-compatible with historical corrected
results; new results use the raw selected blank.
